...
**Path + schema (SSOT is code, not this file).** `~/.config/session-orchestrator/owner.yaml` — per-user, per-machine, never inside any repo, never committed. The path resolver is `resolvePrivateConfigDir()` in `scripts/lib/config/private-config-dir.mjs` (`SO_CONFIG_HOME` > `XDG_CONFIG_HOME` > homedir), wrapped as `resolveOwnerYamlPath()`; schema, defaults, and loading live in `scripts/lib/owner-yaml.mjs` (`resolveOwnerYamlPath`, `loadOwnerConfig`, `writeOwnerConfig`, `getDefaults`). There is no separate slot-resolver module: session-start reads `owner.yaml` through `loadOwnerConfig()` and applies `efficiency.output-level` / `tone.style` when it renders `skills/session-start/soul.md` — in-memory each session, never persisted. First-run interview / reset run via `/bootstrap` (`scripts/lib/owner-interview.mjs`) and `/bootstrap --owner-reset`.
