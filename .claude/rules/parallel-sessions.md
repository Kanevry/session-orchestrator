...
**Second claim class: DONE/GATE verdicts (w4-1).** The scanner also flags a completion or gate-green assertion — `STATUS: done`, "tests pass", "Tests: PASS", "alles grün" — carrying no RUN RECEIPT (a pass/fail count, a typecheck file count, or an exit code) anywhere in the report and no adjacent measurement command. These records carry `kind: "gate-verdict"`; the distributional ones carry `kind: "distributional"` (a record with no `kind` field predates the split and is distributional by construction). The class exists because the scanner was structurally blind to exactly the two line shapes where "done" and "green" get claimed: measured 2026-09-19 over a 130-case labelled done-claim corpus, it answered "no violation" on 128 of 130 — accuracy 30.2%, **Cohen κ 0.007, indistinguishable from guessing** — because its #1198 gate-summary exemption skipped `STATUS:` and "N passed / M failed" lines before any pattern ran. That exemption is kept for the distributional patterns (it removed 46.5% of a 400-event false-positive sample); the new class reads those lines for a verdict instead of a number.
