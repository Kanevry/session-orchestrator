...
Keep `header` at **12 codepoints** where the language allows it — that is the tool's own stated budget, and 30 of 72 template questions break it. It is a target, not a wall: the 12 lives only in the schema's prose (no `.max(12)`, no truncating render path in bundle 2.1.268), and 125 longer headers were accepted and answered (measured 2026-09-11). So a German compound that lands at 13 or 14 ships; padding it out with an abbreviation nobody reads does not. Templates are held to the 12 by `scripts/auq-audit.mjs`, where shortening costs nothing. Give an option a `preview` when the options differ in something literal — a diff, a title, a config block, a file list: if the answer puts that text somewhere, he sees it first.
