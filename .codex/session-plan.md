...
Acceptance: real vulnerabilities/false positives covered by behavior tests; recovery argv has exact repository/tag and unknown states inspect only; public docs match shipped 4.0.1+ capabilities and installation; no unverified superiority claims; no duplicate directory submissions. Third-party outreach is drafted only (user asked to consider where to submit, not explicitly send messages).
