...
describe('tokenizeCommand — redirect operators (#965)', () => {
  it.each([
    ['> without surrounding whitespace', 'ls>a', ['ls', '>', 'a']],
    ['>> append', 'ls >> a', ['ls', '>>', 'a']],
    ['>| noclobber override is ONE token', 'ls >| a', ['ls', '>|', 'a']],
    ['N> keeps the fd with the operator', 'ls 2> /dev/null', ['ls', '2>', '/dev/null']],
    ['N> without whitespace', 'ls 2>/dev/null', ['ls', '2>', '/dev/null']],
    ['< input redirect', 'sort < a', ['sort', '<', 'a']],
  ])('%s', (_name, command, expectedTexts) => {
    expect(tokenizeCommand(command)).toEqual(
      expectedTexts.map((text) => ({ text, quoted: false })),
    );
  });

  it('keeps &> splitting on & so the chain-operator branch is never shadowed', () => {
    // Load-bearing (#965 Risk C): making `&>` a single token would remove the
    // segment split that keeps `rm -rf /tmp/x &> log` ALLOWED under the rm-rf
    // path-allowlist. The `;`/`|`/`&` branch must win.
  ...
  it('leaves 2>&1 splitting on & exactly as before (fd-dup is not merged)', () => {
