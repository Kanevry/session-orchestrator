...
## v3.2 Autopilot Phase B-3 + B-4 — Backlog Signal + Accuracy Feedback Loop (2026-04-25)

Third feature session in 24 hours on Epic #271 (v3.2 Autopilot). Phase B contract is now **complete** — `signals.backlog` is no longer inert and a `mode-selector-accuracy` learning is now written after every user confirm/override on the Phase 7.5 banner. Phase B no longer has any open follow-ups; Phase C (#277, `/autopilot` Loop Command) is now the only remaining gate for the autonomous-loop epic. Session ran fully coordinator-direct (RAM 1.2 GB free, 7 Claude procs at start — 8th-consecutive isolation:none + enforcement:strict session).

- **`scripts/lib/backlog-scan.mjs` (new, 168 lines)** — exports `scanBacklog({limit, vcs, nowMs}) → {criticalCount, highCount, staleCount, byLabel, total, vcs, limit} | null`, plus `summarizeIssues` (pure aggregator, dependency-injectable for tests), `detectVcs` (origin URL → `github`/`gitlab`/`null`), `clearBacklogCache`, and `STALE_THRESHOLD_DAYS = 30`. VCS detection follows `skills/gitlab-ops/SKILL.md` (origin URL contains `github.com` → `gh`, else `glab`). Module-level `_cache` keyed on `JSON.stringify({vcs, limit})` — one VCS round-trip per session. Graceful-degradation paths return `null` (never throw): missing CLI (caught via spawnSync `r.status !== 0` since ENOENT yields null status), non-zero exit, empty stdout, unparsable JSON, non-array response, no git origin. Explicit `vcs: null` opt-in for tests via `'vcs' in opts ? opts.vcs : detectVcs()` (distinguishes "user didn't pass vcs" from "user explicitly forced null"). Stale = `(now - updated_at) > 30d`; missing/unparsable timestamps count as stale (Infinity convention). Label parsing tolerant: glab returns `["string"]`, gh returns `[{name: "string"}]` — both supported in one pass.
- **`scripts/lib/mode-selector-accuracy.mjs` (new, 116 lines)** — exports `recordAccuracy({recommended, chosen, sessionId, filePath?, nowMs?, expiryDays?, confidence?}) → {ok: true, entry} | {ok: false, reason}`, plus `buildSubject(rec, chosen) = '<rec>-selected-vs-<chosen>'`, `LEARNING_TYPE = 'mode-selector-accuracy'`, `INITIAL_CONFIDENCE = 0.5`, `DEFAULT_EXPIRY_DAYS = 30`. Subject pattern encodes outcome class — `feature-selected-vs-feature` (agreement) and `feature-selected-vs-deep` (override) land at distinct subjects so they accumulate independently under the existing learning lifecycle (+0.15 confirm / -0.20 contradict applied later by `evolve` and `session-end` on dedupe-by-(type, subject)). Helper validates `isValidMode` for both `recommended` and `chosen` (uses the same enum as `recommendations-v0.mjs`); returns structured `{ok: false, reason}` on `no-recommendation` / `invalid-mode-type` / `unknown-mode` / `missing-session-id` / `append-failed: ...` so callers (session-start) log to sweep.log without halting the session. Never throws.
- **`skills/session-start/SKILL.md` Phase 7.5 (+71 lines)** — three integration points: (1) Step 1 imports `scanBacklog` and replaces `// reserved for Phase B-3 — pass null for now` with a live `await scanBacklog({limit: 50})` call, threading the result into `signals.backlog`; (2) Step 5 graceful-no-op table gains two new rows for `glab`/`gh` CLI missing and no-git-origin, both contributing 0 delta; (3) new **Step 6: Record Mode-Selector Accuracy Learning (post-AUQ, Phase B-4)** documents the `recordAccuracy` call shape, trigger conditions (recommendation rendered + active confirm/override + valid modes), skip-silently rules, and sweep.log breadcrumb on failure. Source-bindings table updated (`backlog` → Phase 7.5 Step 1). References block updated with `#293`/`#294` issue links and module paths.
- **Tests (`tests/lib/backlog-scan.test.mjs`, `tests/lib/mode-selector-accuracy.test.mjs`)** — 29 new tests across two files. backlog-scan: pure-aggregator boundaries (stale = exactly 30d → not stale, > 30d → stale, missing date → stale), label-shape tolerance (string vs `{name}`), defensive non-object skipping, unparsable date convention, cache hit returns identical reference, explicit `vcs: null` short-circuit. mode-selector-accuracy: subject format (agreement + override), type-taxonomy constants, happy paths (agreement insight = "confirmed", override insight = "overrode", expiry = +30d, custom confidence override), graceful no-ops (5 reasons), multi-write JSONL accumulation. Coverage: backlog-scan 88.05/82.6/100/92.72; mode-selector-accuracy 96.55/80.64/100/96.55.

Tests: 1712 → 1741 passing (+29 net), 10 skipped. Full Gate green: typecheck 41 OK (+2 modules), lint clean (1 `==` → explicit-`null/undefined/empty` mechanical fix coord-direct before gate, same idiom-vs-eqeqeq pattern as Phase B-1+B-2), coverage **72.94/68.31/78.26/75.08** (was 72.52/67.85/77.75/74.63) — all above thresholds (70/65/70/60). Session ran fully coordinator-direct (cap=0 honored throughout); 5 waves × 0 agents = 0 dispatches.

Forward path: Phase B is closed. The next live Phase 7.5 invocation will exercise `signals.backlog` end-to-end and (post-AUQ) write the first `mode-selector-accuracy` learning — the same dogfooding cadence used for Phase A and Phase B-1+B-2. Phase C (#277) `/autopilot` Loop Command is the next major epic and the only remaining gate for autonomous session orchestration.

## v3.2 Autopilot Phase C — /autopilot Loop Command Scaffold (2026-04-25)

Fourth Phase of Epic #271 (v3.2 Autopilot) and the only remaining gate for autonomous session orchestration. This scaffold ships the contract surface (PRD + skill + command stub) — runtime implementation lives in Phase C-1 follow-up. Session ran fully coordinator-direct (8th-consecutive isolation:none + enforcement:strict) with cap=0 enforced throughout; resource verdict critical at start (RAM 0.7 GB free, 9 Claude procs). 5 waves × 0 agent dispatches = 0 hooks fired. The first `mode-selector-accuracy` learning was written this session (`feature-selected-vs-deep` — user overrode `selectMode`'s passthrough at confidence 0.4 with explicit `deep`).

- **`docs/prd/2026-04-25-autopilot-loop.md` (new, 246 lines)** — full PRD covering: Problem (manual session orchestration costs 10–60s of attention switch per session × 5–8 sessions/day), Non-Goals (no auto-PR/merge, no cross-repo, no SSH/Phase D, no Mode-Selector tuning), Contract (CLI `/autopilot [--max-sessions=N] [--max-hours=H] [--confidence-threshold=0.X] [--dry-run]` with default 5 / 4.0 / 0.85 + silent-clamp bounds), Loop Semantics (per-iteration: max-hours check → resource-overload check → selectMode → confidence-threshold gate → resource-adaptive cap → run_session → kill-switch checks → next iter), Output Schema (`autopilot.jsonl` one record per invocation, optional `autopilot_run_id` cross-reference field added to `sessions.jsonl` entries — null/omitted on manual sessions per v1 additive convention), Resource-Adaptive Concurrency (4-tier cap-decision green/warn/degraded/critical with most-restrictive-signal-wins rule and 2 NEW signals: swap usage via `vm.swapusage`/`/proc/meminfo` + macOS `memory_pressure` percentage), 8 Kill-Switches (spiral, failed-wave, carryover-too-high, max-hours-exceeded, max-sessions-reached, resource-overload, low-confidence-fallback, user-abort), 8 Q-Decisions, Validation matrix, 5 Phase C Follow-Up Sub-Issues, 5 Risks (R1: Mode-Selector accuracy unproven — first accuracy learning written today; gating recommendation: ≥ 20 manual sessions before production), Open Questions deferred to Phase C-1.
- **`skills/autopilot/SKILL.md` (new, 213 lines)** — full skill with YAML frontmatter (`user-invocable: true`, tags: phase-c/autopilot/autonomous/loop), Status section flagging scaffold-only (Phase C-1 ships the runtime), Purpose (collapses per-session attention cost when Mode-Selector is confident enough to make routine decisions autonomously; opt-in by design — `/autopilot` never starts itself), Command Surface (default 5 / 4.0 / 0.85 + silent-clamp), Loop Semantics pseudocode mirroring `autopilot.mjs::runLoop` step order, Atomicity Rule (kill-switches checked AFTER each session completes), Kill-Switches table with recovery hints, Resource-Adaptive Concurrency (cap-decision table + most-restrictive-signal-wins), Telemetry contract, Integration with Other Skills (mode-selector / resource-probe / session-{start,plan,end} / wave-executor / session-registry / mode-selector-accuracy), 6 Critical Rules, 5 Anti-Patterns, References block (PRD + Phase A/B PRDs + epic + #277), 3 Open Questions deferred to Phase C-1.
- **`commands/autopilot.md` (new, 47 lines)** — argument-hint frontmatter, Status section flagging scaffold + pre-runtime behavior (prints "Phase C implementation pending — see PRD" message until C-1 lands), Argument Parsing table, delegation to `skills/autopilot/SKILL.md`, 4 Critical Rules.
- **Vault mirror (`vault/01-projects/session-orchestrator/prd/2026-04-25-autopilot-loop.md`)** — PRD-snapshot with vault frontmatter (`type: reference`, `status: verified`, `type/prd` tag — same fix-forward pattern as Phase A and Phase B mirrors). 116-line condensed version with backlinks to Phase A and Phase B PRDs via `[[…]]` Obsidian wiki-links.
- **Vault `context.md`** — new "Key Decisions" line under v3.2 Autopilot section (Phase C scaffold + cap-decision tiers + confidence default 0.85 + Phase C-1 runtime split + Phase C-3 calibration).

**Concurrency policy (key design call this session):** user pushed back on "hard-block if peers > 0" (would block hours of work when other sessions are merely idle). Replaced with **resource-adaptive cap** based on the most-restrictive of 5 signals (RAM free, swap, CPU load, peer Claude procs, macOS memory_pressure). The 4-tier table (green/warn/degraded/critical) maps cleanly to 6/4/2/0 agent caps. Defaults are conservative initial estimates calibrated against 8-consecutive-coord-direct-success at RAM < 2 GB; Phase C-3 follow-up tunes swap and memory_pressure thresholds against autopilot-run effectiveness data.

Tests: 1741 → 1741 (no delta — Phase C scaffold is doc-only; no new code modules). Full Gate green: typecheck 41 OK (unchanged), lint clean, coverage 72.94/68.31/78.26/75.08 (unchanged). Phase C-1 will ship `scripts/lib/autopilot.mjs` + `tests/lib/autopilot.test.mjs` and `tests/lib/resource-probe-autopilot-signals.test.mjs`.

Forward path: Phase C runtime (`autopilot.mjs::runLoop`) is the next major workstream — Phase C-1 follow-up sub-issue filed. Phase C-2 (resource-probe.mjs swap + memory_pressure signals) and Phase C-3 (cap-decision threshold calibration) are the next two checkpoints. Phase C-4 (`/evolve` type 8 — autopilot-effectiveness learnings comparing manual vs. autopilot completion rates) ships after Phase C-1+C-2 dogfood for ≥ 2 weeks.
