...
- **Active epic:** #271 v3.2 Autopilot — Phase A + B shipped, Phase C-1 + C-1.b + C-1.c + C-2 complete. `scripts/lib/autopilot.mjs` runtime: all 8 kill-switches enforced — 5 pre-iteration + 3 post-session reading `agent_summary.{spiral, failed}` and `effectiveness.{carryover, planned_issues}` off the `sessionRunner` return shape. **Phase C-1.c #301 closed**: `scripts/lib/build-live-signals.mjs` (NEW) extracts the canonical 6-source signals-assembly recipe (state-md, sessions.jsonl, bootstrap.lock, scanBacklog, learnings, vaultStaleness reserved); session-start Phase 7.5 Step 1 swapped from inline pseudocode to `await buildLiveSignals({...})` (behavior-preserving); `skills/autopilot/SKILL.md § Production Wiring` documents the in-process Claude-driven driver protocol with concrete DI snippets. **Phase C-2 #296 closed**: `resource-probe.mjs` extended with `swap_used_mb` (macOS `vm.swapusage` / Linux `/proc/meminfo`) and `memory_pressure_pct_free` (macOS-only `memory_pressure`); `evaluate()` adds `'degraded'` tier between warn and critical with most-restrictive-wins precedence (swap > 3 GB OR mp < 5% → critical; 2-3 GB OR 5-15% → degraded; 1-2 GB OR 15-30% → warn). Live `/autopilot` end-to-end now unblocked. Phase B-1.x quirk fix #299 closed (mode-selector global-max swap: primary recommendation is now the global-max-confidence mode across {primary, ...alternatives}; PASSTHROUGH-only). Phase C-3/C-4 follow-ups (#297/#298) next; Phase C-5 headless-driver follow-up to be filed.
- **Stack:** Node 20+, vitest 4.1.5, ESLint 9. Run `npm ci` after cloning. Test: `npm test`. Lint: `npm run lint`.
- **Tests:** 1851 passed / 10 skipped. Coverage above thresholds 70 / 65 / 70 / 60.
- **Backlog:** ~61 open issues (after closing #301 + #296 this session). Recent meta-audit fallout (#265, #281) closed; #283–#288 discovery/refactor cluster, #297/#298 Phase C calibration/evolve follow-ups remain.
