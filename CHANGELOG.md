...
- **The `is-main-module` entry guard now protects all three lifecycle hooks.** `hooks/on-session-start.mjs`, `hooks/on-session-end.mjs` and `hooks/on-stop.mjs` gate `main()` behind `isMainModule(import.meta.url)` (#1298 P7) — a bare `import()` (a probe, a test, an agent) previously ran `main()` against the live repo and could overwrite `.orchestrator/current-session.json` (incident W4-FX1).
