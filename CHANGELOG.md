...
- The loop-blocked test in `tests/lib/session-start-probes.test.mjs` no longer measures its own instrument (#1 in this session's sweep). Its 50 ms budget sat at 2.5× the meter's own 20 ms sampling floor, so under CI load the cheap probe was recorded `timeout` — pipeline #9459 red, three local runs green. The budget is now 200 ms, still half the blocker's 400 ms, and the assertion pair states the invariant directly: wall-clock passed the budget, the probe's own work did not.
