...
**Argument validation:** Valid scopes: `all`, `code`, `infra`, `ui`, `arch`, `session` (comma-separated for multiple). If any scope is invalid, inform the user: "Invalid scope '[token]'. Valid scopes: all, code, infra, ui, arch, session." and default that token to `all`.
