...
| `--sync-rules` | Copy canonical rules from the plugin's `rules/` library into the consumer repo's `.claude/rules/`. Preserves local rules (files without the plugin source header). |

**Invoke the bootstrap skill.** Read `skills/bootstrap/SKILL.md` and follow its instructions with `INVOCATION_MODE = direct`.

Pass the parsed flags so the skill can skip the tier confirmation question when `--fast`, `--standard`, or `--deep` is provided.
