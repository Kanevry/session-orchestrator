...
- [x] **P1.3** Staleness sweep: a board entry whose backing lease is dead (heartbeat older than the v2 ttl, default 4h) flips `in-progress → force-closed` (never silently drops to closed). _(single-repo case shipped with #674; host-wide propagation shipped 2026-07-02 session-2, #716 — `sweepBoard()`/`buildSweepRepos()` in `board-writer.mjs` wired into session-start Phase 1.7 via `enumerateCandidates`; live-proven: 5 dead foreign leases rendered force-closed from this repo's session-start.)_
...
- [x] **P3.5** New learning type `autonomy-verdict` (`scripts/lib/evolve/autonomy-verdict.mjs`) reusing type-8 `autopilot-effectiveness` + skill-judge signals; reflected via `/evolve` and dialectic peer-cards (AGENT.md "Autonomy Readiness" section). _(main-2026-06-27-deep-1/main-2026-06-30-deep-1: analyzer + tests, `LEARNING_TTL_DAYS` registration, analyzer-only proposal gating, evolve/wave-executor/docs guidance, dialectic AGENT.md readiness rule. Focused analyzer/schema tests and targeted lint passed.)_
...
```
```gherkin
Given ≥1 autopilot run and ≥1 canonical advisory skill-judge judgment exist for repo R
...
- **New learning type:** `autonomy-verdict` in `learnings.jsonl` (schema_version 1 compatible; subject form `<repo-or-scope>-autonomy-readiness`; private scope; 90-day TTL). It is analyzer-only and is not accepted by agent-writable memory proposals.
...
| `autopilot-effectiveness`/`autonomy-verdict` data-gated (needs autopilot runs plus canonical advisory skill-judge judgments) | Verdict weak early | Degrade gracefully to confidence + CI/resource signals only; learning sharpens over time; document the cold-start behavior |
