...
- **Den Always-on-Regelkorpus „unter die Grenze bringen".** *Er ist darunter.* Live gemessen über `computeInstructionBudget()` aus `scripts/lib/instruction-budget-guard.mjs`:
  ...
  totalBytes  113.975 / 114.000  overByteBudget  false
  ```
  Revision 1 verglich eine rohe `wc -c`-Summe (114.758, **mit** Frontmatter) mit einer Grenze für eine Zahl **ohne** Frontmatter und schloss daraus „bereits darüber". Die bindende Achse ist ohnehin nicht Byte, sondern **Direktiven** (`tests/rules/receiving-review.test.mjs:43-51`). Was bleibt, ist eine Randbedingung, kein Arbeitsstrang: **15 Direktiven und 25 Byte Luft** (Stand 2026-08-22) — jede neue Always-on-Regel muss sich das leisten können, und die Zahl wandert, weil `/reconcile` maschinell erzeugte Regeln dorthin schreibt. Seit 2026-08-22 gilt zusätzlich ein harter Paritäts-Wächter: `scripts/lib/validate/check-rules.mjs` scheitert bei `globs:` ohne `paths:` (`kind: missing-paths`) und bei divergierenden Listen (`kind: divergent`) — eine neue Regel muss BEIDE Schlüssel mit identischer Liste tragen.
...
| 2 | VS-3 (Regelkorpus) löste ein nicht existierendes Problem | **Arbeitsstrang gestrichen.** Live: 465/480 Direktiven, 113.975/114.000 Byte, `overBudget: false`. Bleibt als Randbedingung |
