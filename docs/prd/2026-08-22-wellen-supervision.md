...
> *Korrektur (2026-08-25):* Der oben zitierte Kommentar und der Anker `:601-602` beschreiben den Baumzustand **zum Zeitpunkt dieser Revision** und sind seit `5ddecc7` (FA-4) überholt — der Befund selbst hält, er ist umgesetzt. Der Block steht heute bei `:616-624` und lautet: *„v2.1.163+ additionalContext: surface the finding inline, not just in stderr + events.jsonl. … The recipient is the SUBAGENT that just stopped — NOT the coordinator."* Die irreführende Zuschreibung an den Koordinator ist damit weg, der tatsächliche Zustellweg dokumentiert (gemessen gegen Claude Code 2.1.241, 2026-08-23), und ein Test prüft den **Ausgabekanal** statt des Kommentartexts (`tests/hooks/post-subagent-discovery-validator.test.mjs:376-384`). Der ursprüngliche Wortlaut bleibt hier als datierter Befund stehen — dies ist eine Korrektur des Ankers, keine Umschrift der Geschichte.
...
- [x] **FA-3 — Das Dispatch-Experiment. ERLEDIGT 2026-08-22** (§ 1e). Ergebnis: 143 s nicht handlungsfähig; Eskalations-Latenz = Restlaufzeit des Batches. ~~Verbleibende Arbeit: die ADR-Zeile in `docs/adr/0010-native-autonomy-commands.md`.~~ *Korrektur (2026-08-25): auch die ADR-Zeile steht — `docs/adr/0010-native-autonomy-commands.md:92` trägt die 143-s-Messung samt der daraus folgenden FA-6-Konsequenz und einem Revisit-Trigger.*
- [x] **FA-1 — Transkript-Tail. ERLEDIGT 2026-08-25.** Ein Out-of-Process-Beobachter folgt den live wachsenden `subagents/agent-*.jsonl` der aktuellen Welle und bringt das **bestehende** `stagnation_detected`-Ereignis zum Feuern. Drei Detektoren. *Beleg:* `scripts/lib/wave-transcript-tail.mjs` existiert (24.809 B), `grep -c stagnation_detected` → **8**.
- [x] **FA-2 — Agent→Koordinator-Rückkanal. ERLEDIGT 2026-08-25.** Die fünf **schreibenden** Agenten plus `session-reviewer` dürfen einen wellenblockierenden Hinderungsgrund per `SendMessage` an `main` melden. Setzt **#1051** um; dieser PRD ergänzt die Koordinator-Seite. *Beleg:* `grep -ln SendMessage agents/*.md` → **7** Dateien: die fünf schreibenden (`code-implementer`, `db-specialist`, `ui-developer`, `test-writer`, `docs-writer`), `session-reviewer` und die Autorenkonvention `AGENTS.md`.
- [x] **FA-6 — Die Prohibition kippen. ERLEDIGT 2026-08-25.** `run_in_background: true` in Wellen erlauben. **Der riskanteste Teil, und er ist mehr als eine Textänderung:** der heutige Batch-Schutz (3–4 Calls je Message, wartend) verhindert das dokumentierte „große Fan-outs verschlucken Agent-Calls still" (Konfidenz 1,0, 5 Sessions). Er muss **ersetzt** werden, nicht entfernt — Dispatch-Verification muss gegen die Menge der *gestarteten* Agenten prüfen, nicht gegen die Menge der zurückgekehrten Tool-Results. *Beleg:* `wave-loop.md:104` und `:540` tragen jetzt „**`run_in_background: true` is ALLOWED and RECOMMENDED for wave dispatch**" samt der 143-s-Messung; der Ersatz-Guard steht als § Started-Set Verification (`:135`) — gestartet = `meta.json`-Sidecar, abgeschlossen = Task-Notification, nie der Launch-Ack.
- [x] **FA-7 — Veraltete Aussagen korrigieren. ERLEDIGT 2026-08-25** (Operator-Entscheidung A5). Zwei Ziele in diesem Schnitt: (a) `skills/_shared/platform-tools.md` gegen die gemessene Codex-Realität; (b) die 6 Pin-Stellen samt ihrer widerlegten Begründung. Das dritte Ziel ist FA-4, das vierte (`express-path`) liegt im Schwester-PRD als VS-1. *Beleg (a):* `platform-tools.md:28` nennt das collaboration-Toolset (`spawn_agent`, `list_agents`, `wait_agent`, `send_message`, `followup_task`, `interrupt_agent`, `close_agent`) mit Messdatum **2026-08-25 auf `codex-cli 0.141.0`**; `grep -n "otherwise execute sequentially"` → **0 Treffer**. *Beleg (b):* `grep -rn "ALWAYS use \`run_in_background: false\`" skills/ agents/ .claude/` → **0 Treffer**.
- [x] **FA-4 — Fehladressierungs-Fix. ERLEDIGT 2026-08-25** (`5ddecc7`). `post-subagent-discovery-validator.mjs`: den irreführenden Kommentar entfernen, den tatsächlichen Zustellweg dokumentieren, und einen Test auf den **Ausgabekanal** (nicht auf den Kommentartext) legen. *Beleg:* `grep -n "feed the warning back to the coordinator"` → **0 Treffer**; `:38` dokumentiert jetzt `additionalContext` → *the STOPPING SUBAGENT*; Test bei `tests/hooks/post-subagent-discovery-validator.test.mjs:376-384`.
- [x] **FA-5 — Observer-Beobachtungsposten. ERLEDIGT 2026-08-25.** ADR-Eintrag zum nativen `observer:`/`ObserverReport`-Fund samt seiner zwei Sperren. *Beleg:* `docs/adr/0010-native-autonomy-commands.md:90` — „Observer agents = watch-item; a null runtime probe refutes NOTHING", beide Statsig-Gates plus die Worker-statt-Koordinator-Sperre benannt, mit Revisit-Trigger.
...
> *Korrektur (2026-08-25):* Die Zeilenanker dieser Tabelle sind Momentaufnahmen der Revision und inzwischen gewandert — der FA-6-Eintrag `skills/session-start/SKILL.md:1210` steht heute bei **`:1221`**. Die Prohibition dort ist umgesetzt gekippt und lautet jetzt *„`run_in_background: true` is allowed and recommended for wave dispatch"* mit Verweis auf `wave-loop.md § Started-Set Verification`; ein `grep -rn "ALWAYS use \`run_in_background: false\`" skills/ agents/ .claude/` findet 2026-08-25 **keine** Fundstelle mehr. Anker in diesem Dokument sind als Fundhilfe zu lesen, nicht als Zusicherung — vor jeder Nutzung neu greppen.
