...
- **`wave-executor`'s in-process dispatch model** (`skills/wave-executor/wave-loop.md`, learning `parallel-subagent-dispatch-5w6a` conf 0.95). The skill does not attempt to host wave agents in panes — they remain harness-managed Agent() calls.
