...
The grant is exactly one directory: containment is exact-prefix on the realpath-resolved candidate (both the raw and the canonical project root are encoded, since Claude Code names the directory after the path it was launched with), so a SIBLING repo's memory dir, an adjacent-prefix sibling directory (`<memory>-evil/`), a `..` traversal out of it, and every other out-of-repo path stay denied. One honest caveat: the harness name is not unique — `encodeProjectDir()` maps both `/` and `.` to `-`, so two repo roots differing only in `.` vs `-` (`/x/a.b` and `/x/a-b`) share ONE memory dir. That collision is the harness's, inherited here and not narrowable by this gate — and in-repo enforcement, including Discovery's `allowedPaths: []` deny-all, is untouched. The encoder is bound lazily and a load failure falls through to the deny (fail-closed). **What the grant does NOT distinguish:** the hook cannot tell the coordinator from a dispatched subagent (both carry the same raw session id), so EVERY wave agent — including a Discovery agent under `allowedPaths: []` — may write this directory, whose content is auto-injected as trusted project context into every later session, outside `git diff`, CI review, owner-leakage and gitleaks. That exposure is tracked as #1352 (`priority::high`); until it lands, treat `orchestrator.scope.memory_dir_allowed` as the audit trail to read after any wave.
