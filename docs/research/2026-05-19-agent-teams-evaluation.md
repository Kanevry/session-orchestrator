...
- **Parallel dispatch.** `skills/wave-executor/wave-loop.md § Dispatch Agents` — small-batch parallel `Agent()` dispatch (3–4 calls per message; updated 2026-07-03 per #724 from the former single-message fan-out — line numbers in this 2026-05-19 snapshot have since drifted); optional bounded worker-pool via `scripts/lib/wave-executor/pool.mjs` (`SKILL.md`, `worker-pool.enabled`).
