...
1. #1368: STATE.md-Init schreibt `started_at` aus `session.lock` (Helper `resolveSessionStartedAt`) **und** `session-id:` (raw UUID, additiv); `commands/close.md` <!-- path-check: historical --> fällt auf `readLock().session_id` zurück; Reader bekommt 6-h-Toleranz mit Negativtest (9 h → abgelehnt).
