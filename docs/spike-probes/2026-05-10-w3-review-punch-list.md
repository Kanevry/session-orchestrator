...
2. **Schema-version v1→v2 stance reconciliation** (C6 #1, #2): currently §Thin-slice item 1 declares the bump but Risk #1 defers it. Pick one — recommend Risk #1 wins ("additive-only first; bump in follow-up after all 82 historical entries readable with new validator"). Update item 1 prose to match. **Status (2026-07-02): done — the deferred bump landed via #372 (135/135 production entries validate); see the status note in ADR-364 §Thin-slice item 1.**
