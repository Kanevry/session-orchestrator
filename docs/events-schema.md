...
| `orchestrator.vault.mirror_completed` | `scripts/vault-mirror.mjs` (`emitMirrorEvent`) | **failure-only today.** Both `skipped-invalid` branches: validation and mapper-crash. Until #1073 these dropped a record with `{"action":"skipped-invalid"}` on stdout and exit 0 — the affected session simply had no vault note and nobody was told. **Payload:** `action`, `kind`, `line`, `record_id`, `skip_class` (`validation`\|`mapper-crash` — mirrors the stdout `reason` value verbatim so the failure class stays groupable without string-matching), `reason` (the renderer's message, clamped to 300 chars), `dry_run`. `record_id`/`skip_class`/`reason` treat **`null` as not-measured** and are omitted: a `record_id: null` would read as "measured, empty id" rather than "this record had none". **`path` is never emitted** — both sites are reached BEFORE a target path resolves. Called 2-arg (no `repoRoot`): this CLI has no repo-root concept, only `--source`, and deriving one would split a single run's events across two ledgers. **Not yet wired:** the 19 `emitAction()` success-path sites in `scripts/lib/vault-mirror/process.mjs` (`skipped-noop` 5, `skipped-handwritten` 5, `updated` 2, `skipped-quality-low` 2, `created` 2, `skipped-collision-resolved` 1, `skipped-abandoned` 1, census 2026-08-23) — the `path` field is only reachable from there. **`quality_gate` deliberately does NOT exist on this event:** the thresholds are parsed in `vault-mirror.mjs` but the DECISION is in `scripts/lib/vault-mirror/process.mjs` (`processLearning` for confidence, `processSession` for narrative length); both processors return `undefined`, so the rejection is unobservable from the CLI and a field here could never be populated (scaffolding — `build-value.md` BV-001.1). Whoever wires `process.mjs` should feed it from the string that already exists as `meta.reason` = `confidence:X < min:Y` / `narrative:N < min:M`) rather than recomputing it — recomputing is how one fact becomes two copies |
