...
function validateFrontmatter(fm, name, path) {
  ...
  // `user-invocable` is deliberately NOT a hard fail on a non-boolean: a quoted
  // or cased value (`"true"`, `True`) used to crash this generator while the
  // Cursor and Pi generators emitted a wrapper for it and the counters ignored
  // it — four readings of one key. `isUserInvocableValue` is now the single
  // normaliser; a value it cannot read as `true` demotes the skill (with its own
  // WARN for the truthy-looking cases) instead of stopping the build.
  if (Object.hasOwn(fm, 'user-invocable')
    && typeof fm['user-invocable'] !== 'boolean'
    && typeof fm['user-invocable'] !== 'string') {
    fail('user-invocable must be a boolean (or the string form `"true"`/`"false"`)');
  }
...
/**
 * The repo-wide marker for "operator-facing slash command" — delegated to
 * `isUserInvocableValue`, the ONE normaliser, so this generator, the Cursor and
 * Pi generators and the counters cannot disagree about a quoted or cased value.
 *
 * @param {Record<string, unknown>} fm
 * @param {string} [path] named in the shared WARN when a truthy-looking value is demoted
 * @returns {boolean}
 */
function isUserInvocable(fm, path) {
  return isUserInvocableValue(fm?.['user-invocable'], path);
}
  ...
  for (const directory of ['skills', 'commands']) {
    ...
    for (const entry of readdirSync(directoryPath, { withFileTypes: true })) {
      ...
      const policy = command
        ...
        : (readNativePolicy(root, name) ?? (isUserInvocable(fm, path) ? fm['disable-model-invocation'] !== true : undefined));
