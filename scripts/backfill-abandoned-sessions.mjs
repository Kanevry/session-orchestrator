 ...
 *
 * @param {{ repoRoot: string, apply: boolean }} args
 * @returns {Promise<object>} aggregate summary
 */
export async function runMigration({ repoRoot, apply }) {
...
async function main() {
  ...
  if (values.help) {
    process.stdout.write(
      'Usage: node scripts/backfill-abandoned-sessions.mjs [--dry-run|--apply] [--json] [--repo-root PATH]\n' +
  ...
  let summary;
  try {
    summary = await runMigration({ repoRoot, apply });
  } catch (err) {
