 ...
 *   2. `.agents/skills/<name>/SKILL.md` — a portable mirror of each skill and
 *      command (commands own overlapping names), carrying ONLY spec-legal
 *      frontmatter plus a pointer body (progressive disclosure — the mirror
 *      never duplicates the canonical instructions). Native Codex invocation
 *      policy travels in an adjacent `agents/openai.yaml` when applicable.
 *
...
const POLICY_MARKER = '# Generated by scripts/generate-agents-skills.mjs; do not edit.\n';
 ...
 * @param {string} [canonical] plugin-relative source path; defaults to the skill
 * @returns {string}
 */
export function renderMirror(skillName, portableFm, canonical = `skills/${skillName}/SKILL.md`) {
  const front = yaml.dump(portableFm, { lineWidth: -1, noRefs: true, quotingType: '"' });
  const command = canonical.startsWith('commands/');
  const lines = [
    '---',
    front.trimEnd(),
    '---',
  ...
  if (command || isUserInvocableValue(portableFm.metadata?.['user-invocable'])) lines.push(
    '',
...
/** Parse a source's YAML frontmatter block; returns null when absent/unparseable. */
...
/** Union of canonical skills and commands, preserving command precedence. */
function listSourceEntries(pluginRoot) {
  const sources = new Map(listSourceSkills(pluginRoot).map((name) => [name, `skills/${name}/SKILL.md`]));
  const commands = path.join(pluginRoot, 'commands');
  if (existsSync(commands) && statSync(commands).isDirectory()) {
...
/** Match Codex policy precedence: command, native skill override, public skill. */
function invocationPolicy(pluginRoot, name, canonical, fm) {
  if (!canonical.startsWith('commands/')) {
    const nativePath = path.join(pluginRoot, 'skills', name, 'agents', 'openai.yaml');
    if (existsSync(nativePath)) {
      const native = yaml.load(readFileSync(nativePath, 'utf8'), { schema: yaml.CORE_SCHEMA });
      const isRecord = (value) => value !== null && typeof value === 'object' && !Array.isArray(value);
      if (!isRecord(native) || (Object.hasOwn(native, 'policy') && !isRecord(native.policy))) {
        throw new Error(`${nativePath}: native skill metadata and policy must be mappings`);
      }
      if (native.policy && Object.hasOwn(native.policy, 'allow_implicit_invocation')) {
        if (typeof native.policy.allow_implicit_invocation !== 'boolean') {
          throw new Error(`${nativePath}: policy.allow_implicit_invocation must be a boolean`);
        }
        return native.policy.allow_implicit_invocation;
      }
    }
    if (!isUserInvocableValue(fm['user-invocable'], canonical)) return undefined;
  }
  if (Object.hasOwn(fm, 'disable-model-invocation') && typeof fm['disable-model-invocation'] !== 'boolean') {
    throw new Error(`${canonical}: disable-model-invocation must be a boolean`);
  }
...
export function generateAgentsSurface({ pluginRoot, check = false }) {
  ...
  const sources = listSourceEntries(pluginRoot);
  ...
  for (const name of skills) {
    ...
    const src = path.join(pluginRoot, canonical);
    const fm = readFrontmatter(src);
    if (fm === null) {
      warnings.push(`${canonical}: has no parseable frontmatter — skipped`);
      continue;
    }
    const { fm: portable, warnings: w } = toPortableFrontmatter(fm, name);
    warnings.push(...w);
    const artifacts = new Map([['SKILL.md', renderMirror(name, portable, canonical)]]);
    const policy = invocationPolicy(pluginRoot, name, canonical, fm);
    const policyPath = path.join(mirrorRoot, name, 'agents', 'openai.yaml');
    if (policy !== undefined) {
      artifacts.set('agents/openai.yaml', POLICY_MARKER + yaml.dump({
        interface: { display_name: name },
        policy: { allow_implicit_invocation: policy },
      }, { lineWidth: -1, noRefs: true }));
    } else if (existsSync(policyPath) && readFileSync(policyPath, 'utf8').startsWith(POLICY_MARKER)) {
      if (check) drift.push(`.agents/skills/${name}/agents/openai.yaml is obsolete`);
      else {
        rmSync(policyPath);
        written.push(`removed .agents/skills/${name}/agents/openai.yaml`);
      }
    }
    for (const [file, rendered] of artifacts) {
      const relative = `.agents/skills/${name}/${file}`;
      const dest = path.join(mirrorRoot, name, file);
      files += 1;
      bytes += Buffer.byteLength(rendered, 'utf8');
      const current = existsSync(dest) ? readFileSync(dest, 'utf8') : null;
      if (file === 'agents/openai.yaml' && current !== null && !current.startsWith(POLICY_MARKER)) {
        drift.push(`${relative} conflicts with an unrelated native policy — retained`);
        continue;
      }
      if (current === rendered) continue;
      if (check) drift.push(`${relative} is ${current === null ? 'missing' : 'stale'}`);
      else {
  ...
  if (existsSync(mirrorRoot)) {
    ...
    for (const entry of readdirSync(mirrorRoot, { withFileTypes: true })) {
      ...
      if (check) {
        drift.push(`.agents/skills/${entry.name}/ is an orphan (no skill or command source)`);
      } else {
...
function main(argv) {
  ...
  if (args.json) {
    ...
    process.stdout.write(
      `agents-surface: ${result.written.length} file(s) written, ${result.files} portable artefact(s), ${result.bytes} bytes\n`,
    );
  }
