...
// Scalar activation keys captured (issue #694). The `globs` key keeps its
// dedicated sequence/flow handling and is NOT routed through this table.
...
/**
 * Deterministic gating check (issue #694). Returns `{ excluded: boolean,
 ...
 */
function applyGates(meta, filePath, mode, hostClass, now) {
...
export function loadApplicableRules({
  ...
  for (const entry of entries) {
    ...
    // Deterministic gating (issue #694) — applies to BOTH always-on and
    // glob-matched candidates. Parse-error rules carry empty meta, so they pass
    // every gate (preserving fail-open behavior).
    if (!parseError && applyGates(meta, filePath, mode, hostClass, now).excluded) {
      continue;
    }
