...
- **A linked worktree makes `.git` a file, not a directory.** 13 tracked files assume a directory and flag it as "not in repository" even in an unmodified worktree at the same layout (`scripts/lib/workspace.mjs` resolves the repo root through the file-link form) — that is the worktree artefact, not a defect to fix.
