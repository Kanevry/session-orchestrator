...
> - `waves[].suite_passed` / `waves[].suite_failed` / `waves[].suite_platform` (#944): OPTIONAL per-wave fields, populated from STATE.md Wave History headers of the form `— suite <passed>/<failed> on <platform>` (written by wave-executor §3a since #944); omit all three when absent (pre-#944 sessions / no full suite ran in that wave) — absent = "not measured", `suite_failed: 0` = "measured, zero failures".
>   **The STATE.md header is still the source — deliberately, for now (#957/F1).** Since #954 the gate also emits a machine-measured `counts: {passed, failed, total}` on its `orchestrator.quality_gate.*` event, so this one fact now has two producers and only the prose one has a reader. Do NOT switch this field to the event yet: `waves[].*` is per-wave and gate events carry no `wave_number`, so attributing an event to a wave means a wall-clock window join on `waves[].started_at`/`completed_at` — boundaries that are themselves LLM-written, i.e. one LLM hop traded for another (`scripts/lib/eval/session-resolve.mjs` already documents window-attributed gate events as unsafe under a contaminated window). **Remaining work to retire the prose path, in order:** (1) add `wave_number` to the gate event payload in `scripts/run-quality-gate.mjs` (the wrapper that actually runs between waves) and `scripts/lib/quality-gate.mjs`; (2) have wave-executor pass the current wave number to the gate invocation; (3) only then repoint this field at the event and drop the hand-written trio from `wave-loop.md` step 7.
