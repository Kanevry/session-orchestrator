...
7. **Capture wave metrics**: If `persistence` is enabled in Session Config, record for this wave after all agents complete and quality checks run. If `persistence` is `false`, skip metrics capture entirely — do not accumulate in-memory metrics. Record:
   ...
   - `suite_passed` / `suite_failed` (+ optional `suite_platform`): the full-suite counts from the SAME gate run that feeds the § 3a Wave History header `— suite <passed>/<failed> on <platform>`. `quality_check` is a traffic light; these are the number the light was derived from, and unlike STATE.md (gitignored, demoted to `## Previous Session` and then overwritten) the metrics record survives the session. **OMIT all three when no full suite ran in this wave** — absent = "not measured", `suite_failed: 0` = "measured, zero failures". Never write `0` for a suite that did not run.
     > Since #954 the gate ALSO emits a machine-measured `counts: {passed, failed, total}` on its `orchestrator.quality_gate.*` event. That is a second emission of the same fact, not a replacement: this hand-written value remains the one session-end reads (see `skills/session-end/metrics-collection.md` § 1.7 for why, and for the remaining work). Write it from the gate output as before — do not skip it because the event exists.
